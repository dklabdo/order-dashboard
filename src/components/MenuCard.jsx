...
export default function MenuCard({ item }) {
  return (
    <div className="relative min-w-[150px] sm:min-w-[180px] md:min-w-[216px] w-full max-w-[280px] sm:max-w-[300px] md:max-w-[320px] h-[250px] sm:h-[280px] md:h-[300px] rounded-2xl mt-7 bg-[#1f0b2f] text-white shadow-lg flex flex-col items-center pt-[40px] sm:pt-[50px]">
      <div className="absolute top-[-25px] sm:top-[-33px] left-1/2 transform -translate-x-1/2 z-10 transition-transform duration-300 hover:scale-110">
        <img
          src="/burger.jpg"
          alt={item.name}
          className="w-[100px] h-[80px] sm:w-[120px] sm:h-[90px] md:w-[148px] md:h-[112.35px] object-cover rounded-xl shadow-xl "
        />
      </div>

      <div className="text-center px-3 flex-1 pt-[25px] sm:pt-[30px]">
        <h2 className="text-sm sm:text-md md:text-lg font-semibold-barlow-16px mt-2">{item.name}</h2>
        <p className="text-[0.6rem] sm:text-xs text-gray-300 mt-1 leading-snug">
          {item.ingredients}
        </p>
        <br/>
